...
function openFeature(i){
  ...
  if(!modalEl||!body||!title||!link){ location.hash=f.link; return; }
  title.textContent=f.title; link.href=f.link||'#';
  const widgets={
    chart:`<canvas id="modalChart" height="180"></canvas><p class="small-muted mt-2">Demo-Chart nutzt <code>data/charts.json</code>.</p>`,
...
async function loadNews(){
  ...
  try{ const r=await fetch('partials/top5.html',{cache:'no-store'}); const box=document.getElementById('top5-container'); if(box) box.innerHTML=r.ok?await r.text():'–'; }catch{}
}
function searchSite(){
...
const FAQ=[
  ...
  {q:"Kaution – wie viel & wann zurück?", a:"Max. 3 Nettokaltmieten; Rückzahlung i. d. R. innerhalb 3–6 Monate (Prüffrist)."}
];
...
async function drawDemoChart(id='zinsChart'){
  ...
  series=(charts&&charts.zins)?charts.zins:{labels:["Jan","Feb","Mrz","Apr","Mai","Jun","Jul","Aug","Sep","Okt","Nov","Dez"],data:[2.1,2.4,2.8,3.2,3.5,3.7,3.9,4.0,3.7,3.5,3.4,3.3]};
...
document.addEventListener('DOMContentLoaded',()=>{
  renderFeatures(); renderFAQ(); renderSmart(); renderGallery(); loadNews(); drawDemoChart();
});
